...
class PainelPixKeysViewTest(TestCase):
    ...
    def test_checkout_pix_permanece_disponivel_apos_criar_chave_no_painel(self):
        ChavePix.objects.create(
            restaurante=self.restaurante,
            tipo=ChavePix.Tipo.EMAIL,
        ...
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'checkout@restaurante.com')

    def test_painel_pix_keys_nao_exibe_historico_antes_depois(self):
        self.client.post(
            '/pagamentos/painel/chaves-pix/criar/',
            data={
                'tipo': ChavePix.Tipo.EMAIL,
        ...
        self.assertNotContains(response, 'Depois')
